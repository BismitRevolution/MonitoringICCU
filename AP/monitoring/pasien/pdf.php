...
class PDF extends FPDF{
  function Header(){
    ...
    $this->Cell(20,0,'Data pasien',0,0,'L');
    $this->Ln();
    $this->Cell(5,1,'Laporan data pasien',0,0,'L');
    $this->Ln();
...
$arr=getData($conn,$sql);
		foreach($arr as $d) {	
  $cell[$i][0]=$d["kode_pasien"];
  $cell[$i][1]=$d["nama_pasien"];
...
$pdf->Cell(2,1,'kode_pasien','LR',0,'C',1);
$pdf->Cell(7,1,'nama_pasien','LR',0,'C',1);
$pdf->Cell(5,1,'jenis_kelamin','LR',0,'C',1);
$pdf->Cell(3,1,'alamat','LR',0,'C',1);
$pdf->Cell(9,1,'telepon','LR',0,'C',1);
...
for ($j=0;$j<$i;$j++){
  ...
  $pdf->Cell(2,1,$cell[$j][0],'B',0,'L'); // kode_pasien
  $pdf->Cell(7,1,$cell[$j][1],'B',0,'L'); // nama_pasien
  $pdf->Cell(5,1,$cell[$j][2],'B',0,'L'); // jenis_kelamin
  $pdf->Cell(3,1,$cell[$j][3],'B',0,'L'); // alamat
  $pdf->Cell(9,1,$cell[$j][4],'B',0,'L'); // telepon
  $pdf->Cell(9,1,$cell[$j][5],'B',0,'L'); // catatan
