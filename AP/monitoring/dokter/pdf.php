...
class PDF extends FPDF{
  function Header(){
    ...
    $this->Cell(20,0,'Data dokter',0,0,'L');
    $this->Ln();
    $this->Cell(5,1,'Laporan data dokter',0,0,'L');
    $this->Ln();
...
$arr=getData($conn,$sql);
		foreach($arr as $d) {	
  $cell[$i][0]=$d["kode_dokter"];
  $cell[$i][1]=$d["nama_dokter"];
  $cell[$i][2]=$d["telepon"];
  $cell[$i][3]=$d["email"];
  $cell[$i][4]=$d["username"];
  $cell[$i][5]=$d["password"];
...
$pdf->Cell(2,1,'kode_dokter','LR',0,'C',1);
$pdf->Cell(7,1,'nama_dokter','LR',0,'C',1);
$pdf->Cell(5,1,'telepon','LR',0,'C',1);
$pdf->Cell(3,1,'email','LR',0,'C',1);
$pdf->Cell(9,1,'username','LR',0,'C',1);
...
for ($j=0;$j<$i;$j++){
  ...
  $pdf->Cell(2,1,$cell[$j][0],'B',0,'L'); // kode_dokter
  $pdf->Cell(7,1,$cell[$j][1],'B',0,'L'); // nama_dokter
  $pdf->Cell(5,1,$cell[$j][2],'B',0,'L'); // telepon
  $pdf->Cell(3,1,$cell[$j][3],'B',0,'L'); // email
  $pdf->Cell(9,1,$cell[$j][4],'B',0,'L'); // username
  $pdf->Cell(9,1,$cell[$j][5],'B',0,'L'); // password
