...
<?php  
  $sql="select * from `$tbperawatan` where `kode_dokter`='$kode_dokter' order by `kode_perawatan` desc";
  $jum=getJum($conn,$sql);
		if($jum > 0){
	//--------------------------------------------------------------------------------------------
	$batas   = 10;
...
echo"<tr bgcolor='$color'>
				...
				<a href='?mnu=dlog&id=$kode_perawatan'><img src='ypathicon/iclog.jpg' alt='Log'></a>
				</td>
				</tr>";
...
if($jmldata>0){
	...
	if($page > 1){
		$prev=$page-1;
		echo "<span class=prevnext><a href='$_SERVER[PHP_SELF]?page=$prev&mnu=dperawatan'>« Prev</a></span> ";
	}
	else{echo "<span class=disabled>« Prev</span> ";}

	// Tampilkan link page 1,2,3 ...
	for($i=1;$i<=$jmlhal;$i++)
	if ($i != $page){echo "<a href='$_SERVER[PHP_SELF]?page=$i&mnu=dperawatan'>$i</a> ";}
	else{echo " <span class=current>$i</span> ";}

	// Link kepage berikutnya (Next)
	if($page < $jmlhal){
		$next=$page+1;
		echo "<span class=prevnext><a href='$_SERVER[PHP_SELF]?page=$next&mnu=dperawatan'>Next »</a></span>";
	}
