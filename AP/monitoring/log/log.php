...
<tr>
<td height="24"><label for="kode_pasien">Pasien</label>
<td>:<td colspan="2">
...
<tr>
<td height="24"><label for="kode_dokter">Dokter</label>
<td>:
<td>
...
<?php echo $status; ?>
 ...
 <p>
   ...
   Simpan Log 
  <br>
  <a href="log/pdf.php"><img src='ypathicon/icsave.png' alt='PDF'></a>
   </p>
 <p>Data perawatan: </p>
...
<?php  
  ...
  $jum=getJum($conn,$sql);
	...
	$arr=getData($conn,$sql2);
		foreach($arr as $d) {							
				$id=$d["id"];
				$kp=getPerawatan($conn,$d["kode_perawatan"]);
				$kode_perawatan=$d["kode_perawatan"];
...
if($jmldata>0){
	...
	if($page > 1){
		$prev=$page-1;
		echo "<span class=prevnext><a href='$_SERVER[PHP_SELF]?page=$prev&mnu=dlog&id=$kode_perawatan'>« Prev</a></span> ";
	}
	else{echo "<span class=disabled>« Prev</span> ";}

	// Tampilkan link page 1,2,3 ...
	for($i=1;$i<=$jmlhal;$i++)
	if ($i != $page){echo "<a href='$_SERVER[PHP_SELF]?page=$i&mnu=dlog&id=$kode_perawatan'>$i</a> ";}
	else{echo " <span class=current>$i</span> ";}

	// Link kepage berikutnya (Next)
	if($page < $jmlhal){
		$next=$page+1;
		echo "<span class=prevnext><a href='$_SERVER[PHP_SELF]?page=$next&mnu=dlog&id=$kode_perawatan'>Next »</a></span>";
	}
...
if(isset($_POST["Simpan"])){
	...
	$detak_jantung2=strip_tags($_POST["detak_jantung2"]);
...
$simpan=process($conn,$sql);
		if($simpan) {echo "<script>alert('Data $id berhasil disimpan !');document.location.href='?mnu=dlog&id=$kode_perawatan';</script>";}
		else{echo"<script>alert('Data $id gagal disimpan...');document.location.href='?mnu=dlog&id=$kode_perawatan';</script>";}
	}
	else{
...
`detak_jantung2`='$detak_jantung2' ,
where `id`='$id0'";
$ubah=process($conn,$sql);
	if($ubah) {echo "<script>alert('Data $id berhasil diubah !');document.location.href='?mnu=dlog&id=$kode_perawatan';</script>";}
	else{echo"<script>alert('Data $id gagal diubah...');document.location.href='?mnu=dlog&id=$kode_perawatan';</script>";}
	}//else simpan
}
?>
...
if($hapus) {echo "<script>alert('Data perawatan $id berhasil dihapus !');document.location.href='?mnu=dlog&id=$kode_perawatan';</script>";}
else{echo"<script>alert('Data perawatan $id gagal dihapus...');document.location.href='?mnu=dlog&id=$kode_perawatan';</script>";}
}
?>
