...
<td>:<td colspan="2"><input name="tanggal" type="text" id="tanggal" value="<?php echo $tanggal;?>" size="15" />
</td>
</tr>
...
<td>:
<td><input name="jam" type="text" id="jam" value="<?php echo $jam;?>" size="30" />
  <label for="kode_barang"></label></td>
</tr>

<tr>
<td height="24"><label for="detak_jantung">Detak Jantung</label>
<td>:<td colspan="2"><input name="detak_jantung" type="text" id="detak_jantung" value="<?php echo $detak_jantung;?>" size="25" />
</td>
</tr>

<tr>
<td><label for="note">Catatan</label><td>:<td colspan="2"><textarea name="note" cols="30" rows="3" id="note"><?php echo $note;?></textarea></td></tr>
...
<td colspan="2">	<input name="Simpan" type="submit" id="Simpan" value="Simpan" />
        ...
        <input name="kode_perawatan" type="hidden" id="kode_perawatan" value="<?php echo $kode_perawatan;?>" />
        <a href="?mnu=plog&id=$kode_perawatan"><input name="Batal" type="button" id="Batal" value="Batal" /></a>
</td></tr>
</table>
</form>
...
<?php  
  $sql="select * from `$tblog` where kode_perawatan='$kode_perawatan' order by `id` desc";
  $jum=getJum($conn,$sql);
		if($jum > 0){
	//--------------------------------------------------------------------------------------------
	$batas   = 10;
...
if($jmldata>0){
	...
	if($page > 1){
		$prev=$page-1;
		echo "<span class=prevnext><a href='$_SERVER[PHP_SELF]?page=$prev&mnu=plog&id=$kode_perawatan'>« Prev</a></span> ";
	}
	else{echo "<span class=disabled>« Prev</span> ";}

	// Tampilkan link page 1,2,3 ...
	for($i=1;$i<=$jmlhal;$i++)
	if ($i != $page){echo "<a href='$_SERVER[PHP_SELF]?page=$i&mnu=plog&id=$kode_perawatan'>$i</a> ";}
	else{echo " <span class=current>$i</span> ";}

	// Link kepage berikutnya (Next)
	if($page < $jmlhal){
		$next=$page+1;
		echo "<span class=prevnext><a href='$_SERVER[PHP_SELF]?page=$next&mnu=plog&id=$kode_perawatan'>Next »</a></span>";
	}
...
$simpan=process($conn,$sql);
		if($simpan) {echo "<script>alert('Data $id berhasil disimpan !');document.location.href='?mnu=plog&id=$kode_perawatan';</script>";}
		else{echo"<script>alert('Data $id gagal disimpan...');document.location.href='?mnu=plog&id=$kode_perawatan';</script>";}
	}
	else{
...
$ubah=process($conn,$sql);
	if($ubah) {echo "<script>alert('Data $id berhasil diubah !');document.location.href='?mnu=plog&id=$kode_perawatan';</script>";}
	else{echo"<script>alert('Data $id gagal diubah...');document.location.href='?mnu=plog&id=$kode_perawatan';</script>";}
	}//else simpan
}
?>
...
if($hapus) {echo "<script>alert('Data perawatan $id berhasil dihapus !');document.location.href='?mnu=plog&id=$kode_perawatan';</script>";}
else{echo"<script>alert('Data perawatan $id gagal dihapus...');document.location.href='?mnu=plog&id=$kode_perawatan';</script>";}
}
?>
